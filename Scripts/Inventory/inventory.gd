...
signal item_box_changed(item_box: ItemBox, item_box_index: int)
...
func is_full() -> bool:
	return _find_item_box_index_with_item(null) == -1


func get_item_box(index: int) -> ItemBox:
	if index < 0 or index >= _item_boxes.size():
		return null
	return _item_boxes[index]


func set_item_box(index: int, item: Item, amount: int) -> void:
	if index < 0 or index >= _item_boxes.size():
		return
	_item_boxes[index].set_item_and_amount(item, amount)
	item_box_changed.emit(_item_boxes[index], index)


func clear_item_box(index: int) -> void:
	if index < 0 or index >= _item_boxes.size():
		return
	_item_boxes[index].set_item_and_amount(null, 0)
	item_box_changed.emit(_item_boxes[index], index)
